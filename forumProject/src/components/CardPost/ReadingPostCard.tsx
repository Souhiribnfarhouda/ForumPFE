...
const ReadingPostCard: FC<Props> = ({ id }) => {
  ...
  const [content, setContent] = useState('');
    ...
    `http://localhost:9000/api/getpost/${id}`,
    fetcher
    );
  console.log(post)

  const { data: comments } = useSWR(
    `http://localhost:9000/api/comment/${id}`,
    fetcher
  );
  const handleSubmit = async (e: any) => {
    if (content?.length) {
      ...
      try {
        ...
        await mutate(`http://localhost:9000/api/comment/${id}`);
        setContent('');
      } catch (error) {
        console.error(error);
      }
        ...
        <Stack mt="6" spacing="3">
          <Flex>
            <Flex flex="1" gap="4" alignItems="center" flexWrap="wrap">
              <Avatar name="Segun Adebayo" src={ post?.[0]?.user?.image} />

              <Box>
                <Heading size="sm">{ post?.[0]?.user?.name}</Heading>
            ...
            <IconButton
              ...
              icon={<MdDelete />}
              onClick={async () => {
                await axios.delete('http://localhost:9000/api/post/' + post?.[0]?.id);
                await mutate(
          ...
          <Heading size="md">{post?.[0]?.title}</Heading>
          <Box>{Parser(post?.[0]?.content ?? '')}</Box>
        </Stack>
      </CardBody>
      <CardFooter
        bg={'#f1f1f1'}
...
export default ReadingPostCard;
